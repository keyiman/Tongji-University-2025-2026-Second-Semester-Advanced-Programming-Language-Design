...
void calc_num(char pure_decimal[], char out[], char power_table[][N_NUM + 1])
{
	for (int i = 0; pure_decimal[i] != '\0'; i++)
	{
		if (pure_decimal[i] == '1')
		{
			...
			{
				int sum = out[j] + power_table[i + 1][j] + carry;
				out[j] = sum % 10;
				carry = sum / 10;
			}
		}
...
void input_pure_decimal(char pure_decimal[])
{
	...
	while (1) 
	{
		...
		// 如果长度超过 N_POW (112) 位则只取前 112 位
		int copy_len = (len > N_POW ? N_POW : len);
		for (int i = 0; i < copy_len; i++)
		{
			pure_decimal[i] = temp[i + 1];
		}
		pure_decimal[copy_len] = '\0';

		// 剔除二进制小数末尾所有的无效 0
		int last = copy_len - 1;
		while (last >= 0 && pure_decimal[last] == '0')
		{
			pure_decimal[last] = '\0';
			last--;
		}

		// 剔除完之后，如果变成了空串，说明全都是0！直接报错
		if (pure_decimal[0] == '\0')
		{
			cout << "输入为全0，请重输" << endl;
			continue;
